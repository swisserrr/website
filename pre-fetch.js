...
async function preFetchPages() {
  ...
  for (const category of filteredCategories) {
    try {
      // console.log(`Fetching category page: https://www.emoha.com/blogs/${category.slug}`);
      await fetchWithRetry(`https://www.emoha.com/blogs/${category.slug}`);
    } catch (err) {
      console.error(`Error fetching category page: https://www.emoha.com/blogs/${category.slug}`, err.message);
      continue;
    }
    ...
    for (const post of posts) {
      try {
        // console.log(`Fetching post page: https://www.emoha.com/blogs/${category.slug}/${post.slug}`);
        await fetchWithRetry(`https://www.emoha.com/blogs/${category.slug}/${post.slug}`);
      } catch (err) {
        console.error(
          `Error fetching post page: https://www.emoha.com/blogs/${category.slug}/${post.slug}`,
          err.message
        );
      }
    }

...
import { shellReviewSchema } from 'utils/validationSchema';
import { getPhoneNumberDetails, isValid } from 'utils/phoneUtil';

import makeSelectShellSurvey from './selectors';
import { submitShellData } from './actions';
...
function CustomForm({ isLoading, handleSubmitData, setOpenModal, leadsFrom, noOfElderOptedText, url, heading }) {
        ...
        <div style={{ marginBottom: 20 }}>
          {divPadding(
            <>
              ...
              <CustomInput
                ...
                onChange={event => {
                  onChange(detailObj?.id, event?.target?.value, 'name');
                  if (errors?.parentsDetails?.[index]?.name) setFieldError(`parentsDetails[${index}].name`, '');
                }}
                type="text"
                margin="1rem 0"
          ...
          {divPadding(
            <>
              ...
              <DatePicker
                ...
                onChange={value => {
                  onChange(detailObj?.id, value, 'dob');
                  if (errors?.parentsDetails?.[index]?.dob) setFieldError(`parentsDetails[${index}].dob`, '');
                }}
                value={detailObj?.dob}
                maxDate={new Date()}
              ...
              <IntlTelInput
                ...
                onPhoneNumberChange={(_, phoneNumber) => {
                  ...
                  if (regex.test(phoneNumber)) {
                    ...
                    if (errors?.parentsDetails?.[index]?.phoneNumber)
                      setFieldError(`parentsDetails[${index}].phoneNumber`, '');
                  }
                }}
            ...
            <>
              ...
              <CustomSelect
                ...
                onChange={event => {
                  ...
                  if (errors?.parentsDetails?.[index]?.elder_relationship_with_you)
                    setFieldError(`parentsDetails[${index}].elder_relationship_with_you`, '');
                }}
          ...
          <>
            ...
            <CustomInput
              ...
              onChange={event => {
                onChange(detailObj?.id, event?.target?.value, 'state');
                if (errors?.parentsDetails?.[index]?.state) setFieldError(`parentsDetails[${index}].state`, '');
              }}
              type="text"
              margin="1rem 0"
              placeholder="State"
          ...
          <LocationContainer>
            <CustomInput
              ...
              onChange={event => {
                onChange(detailObj?.id, event?.target?.value, 'city');
                if (errors?.parentsDetails?.[index]?.city) setFieldError(`parentsDetails[${index}].city`, '');
              }}
              type="text"
              margin="1rem 0"
              placeholder="City"
          ...
          <IntlTelInput
            ...
            onPhoneNumberChange={(_, phoneNumber) => {
              ...
              if (regex.test(phoneNumber)) {
                ...
                if (errors.phoneNumber) setFieldError('phoneNumber', '');
              }
            }}
        ...
        divPadding(
          <>
            ...
            <CustomInput
              ...
              onChange={e => {
                handleChange(e);
                if (errors.employee_code) setFieldError('employee_code', '');
              }}
              margin="1rem 0"
              placeholder="Code"
            />
...
export function ShellSurvey({ shellSurvey, handleSubmitData, heading, noOfElderOptedText, url }) {
  ...
  return (
    <>
      ...
      <div
        ...
        {shellSurvey?.isLoading && FullScreenLoader()}
        <HomePageBox>
          <HomePageBox
            display="flex"
            ...
            <HomePageBox width={{ xs: '90%', md: '45%' }}>
              ...
              <Formik
                ...
                validationSchema={shellReviewSchema}>
                <CustomForm
                  handleSubmitData={handleSubmitData}
                  setOpenModal={setOpenModal}
                  isLoading={shellSurvey?.isLoading}
...
const mapStateToProps = createStructuredSelector({
  shellSurvey: makeSelectShellSurvey(),
});

function mapDispatchToProps(dispatch) {
  return {
    dispatch,
    handleSubmitData: (payload, callback) => dispatch(submitShellData(payload, callback)),
  };
}

const withConnect = connect(mapStateToProps, mapDispatchToProps);

export default compose(withConnect, memo)(ShellSurvey);

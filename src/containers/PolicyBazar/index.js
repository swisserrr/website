...
import { policyBazaarReviewSchema } from 'utils/validationSchema';
import { getPhoneNumberDetails, isValid } from 'utils/phoneUtil';

import makeSelectPepsicoSurvey from './selectors.js';
import { submitPepsicoData } from './actions.js';
...
function CustomForm({ isLoading, handleSubmitData, setOpenModal, leadsFrom, noOfElderOptedText, url, heading }) {
  ...
  const handleChangeElderForms = event => {
  ...
  // Automatically trigger onChange with the default value on mount
  useEffect(() => {
    handleChangeElderForms({ target: { value: isEqual(get(router, 'query.type'), '1') ? 1 : 2 } }); // Manually trigger with default
  }, [router]);
    ...
    <Form
      ...
      {divPadding(
        <>
          ...
          <CustomSelect
            value={isEqual(get(router, 'query.type'), '1') ? 1 : 2}
            disabled
            sx={{
              '& fieldset': { border: 'none' },
            }}
            onChange={event => handleChangeElderForms(event)}
            inputProps={{ 'aria-label': 'With label' }}
            // defaultValue={isEqual(get(router, 'query.type'), 1) ? 1 : 2}
            displayEmpty>
            {map(new Array(isEqual(get(router, 'query.type'), '1') ? 1 : 2), (item, key) => (
...
export function PolicyBazar({ pepsicoSurvey, handleSubmitData, heading, noOfElderOptedText, url }) {
          ...
          <HomePageBox
            ...
            <HomePageBox width={{ xs: '90%', md: '45%' }}>
              <HomePageText
                ...
                fontWeight={{ xs: '500', md: '500' }}>
                Policy Bazaar | Emoha Member Onboarding
              </HomePageText>
              <HomePageText
                color={{
                  xs: '#1A1A1A',
              ...
              <Formik
                ...
                validationSchema={policyBazaarReviewSchema(heading)}>
                <CustomForm
                  handleSubmitData={handleSubmitData}
                  setOpenModal={setOpenModal}
                  isLoading={pepsicoSurvey?.isLoading}
...
const mapStateToProps = createStructuredSelector({
  pepsicoSurvey: makeSelectPepsicoSurvey(),
});

function mapDispatchToProps(dispatch) {
  return {
    dispatch,
    handleSubmitData: (payload, callback) => dispatch(submitPepsicoData(payload, callback)),
  };
}

const withConnect = connect(mapStateToProps, mapDispatchToProps);

export default compose(withConnect, memo)(PolicyBazar);
